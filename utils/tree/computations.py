...
# def compute_up_messages(data, tree, evo_model, prob):
#     n_leaves, n_sites = data.shape
#     root = len(tree) - 1
...
#                 slow=prob[node][child]
#                 fast=1-slow
#                 expect=0.1*slow+10*fast
#                 up_table[node] = np.multiply(up_table[node], temp_table)
#                 for i in range (n_leaves):
#                     prev=np.argmax(up_table[node][:,i])
#                     previous=up_table[node][prev][i]
                    
#                     up_table[node][:,i]=up_table[node][:,i]*expect[i]
                    
#                     up_table[node][prev][i]=previous
                    
#                     up_table[node][:,i]= up_table[node][:,i]/(np.sum(up_table[node][:,i]))
                    
#     return up_table


# # compute down messages
# def compute_down_messages(data, tree, evo_model, up_table, prob):
#     n_leaves, n_sites = data.shape
#     root = len(tree) - 1
