...
from utils.data_utils import load_data, load_inference_data
...
HGCN_HOME= os.getcwd()
print(HGCN_HOME)
LOG_DIR= os.path.join(HGCN_HOME, "logs")
os.environ['LOG_DIR'] = LOG_DIR
os.environ['DATAPATH'] = os.path.join(HGCN_HOME, "data")

def train(args, data, n_features):
    ...
    adj_matrix = data['adj_train']
    adj_coo = adj_matrix.tocoo()
    indices = torch.from_numpy(np.vstack((adj_coo.row, adj_coo.col)).astype(np.int64))
    print(indices.shape) 

    with torch.no_grad():
        link_probabilities = model.decode(best_emb, indices.T)
    opts = data['opts']
    
    # create weighted matrix from indices and link_probabilities
    ...
    return  weighted_adj_matrix, labels, opts, model



def train_multiple(args, num_data=20, n_leaves=100, n_features=50):
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)
    if int(args.double_precision):
        torch.set_default_dtype(torch.float64)
    ...
    #load data
    adj, features, labels, opts = generate_data(n_leaves, n_features, alpha=0.1)
    data = load_inference_data(adj, features, labels, opts, args)
    args.n_nodes, args.feat_dim = data['features'].shape
    args.nb_false_edges = len(data['train_edges_false'])
    args.nb_edges = len(data['train_edges'])
    ...
    print(f'Train simulated sequences with {n_leaves} leaves {num_data} times')
    # Train model
    t_total = time.time()
    counter = 0
    best_val_metrics = model.init_metric_dict()
